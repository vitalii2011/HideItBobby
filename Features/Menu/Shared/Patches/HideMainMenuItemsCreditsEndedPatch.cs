...
{
    internal static class HideMainMenuItemsCreditsEndedPatch
    {
        public static readonly PatchData Data = new PatchData(
            patchId: $"SharedPatch.{nameof(HideMainMenuItemsCreditsEndedPatch)}.{nameof(Postfix)}",
            target: () => typeof(MainMenu).GetMethod("CreditsEnded", BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance),
            postfix: () => typeof(HideMainMenuItemsCreditsEndedPatch).GetMethod(nameof(Postfix), BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static),
            onUnpatch: () =>
            {
...
#if DEV
            Log.Info($"{nameof(HideMainMenuItemsCreditsEndedPatch)}.{nameof(Postfix)} setting visibility.");
#endif
